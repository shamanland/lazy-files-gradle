...
class LazyFilesFetchTaskTest {
    ...
    void taskPassAllItemsToUtils() {
        ...
        when(mockUtils.fetch(notNull(String), notNull(LazyFilesItem))).thenReturn(true)
        ...
        verify(mockUtils, times(count)).fetch(eq(mockAccessToken), notNull(LazyFilesItem))
    }
}
